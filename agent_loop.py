...
4. 如果当前环境有匹配的 opencode skill、项目内已有专用脚本，应优先按 skill/脚本的既定流程执行。没有读到对应 skill、文档或脚本前，只能做方案说明，不能直接调用生成。
   ...
   - 如果主持人声音僵硬、机械、过快、过平或像念说明书，必须调整文案和 TTS 参数后重新生成，不能直接写成完成。
    - 电台分段口播必须先建立并保存 `voice_profile.json`，并严格遵守 SKILL.md 中的**声线锁定协议**。`[控制:xxx]` 文本描述不锁定音色，相同描述每次调用可能生成不同声线。必须用以下两种方法之一锁定：
      - **固定 Speaker**：Qwen3-TTS `custom_voice`，所有段用同一个 `speaker`（如 Vivian），每段可用不同 `instruct` 调整情绪，但 speaker 不变 = 音色不变。
      - **克隆锁定**：第一段用 VoxCPM 或 Qwen3 `voice_design` 生成，上传后用 Qwen3 `voice_clone`（`audio_path` + `ref_text`）或 CosyVoice3 `zero_shot`（`prompt_text`）克隆第一段，后续所有段复用同一参考音频。
