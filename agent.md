...
进入本项目后，**第一件事**是检查本地是否有 Soundao 登录凭证（环境变量 `SOUNDAO_API_KEY` 或 `SOUNDAO_USER` + `SOUNDAO_PASS`）。

- **没有凭证时**：立即停止，通过页面进度提示告诉用户："使用 Soundao 需要登录凭证。请加入 QQ 群 1030846851 申请试用，获取 API Key 或账号后发到主 opencode 窗口，我配置好后再继续。" 凭证就绪前，只能读取公开文档、解释能力和整理方案，不能发起任何实际生成、分析或资产调用。
- **有凭证时**：先查询余额确认凭证有效，再继续后续流程。
...
```bash
python soundao-web-agent-framework/soundao_cloud.py llms --path /llms-full.txt --out <输出目录>/llms-full.txt
```
...
如果本地没有可用用户凭证，Agent 不要尝试调用会消耗额度或需要登录的接口。应先通过页面进度提示告诉用户："使用 Soundao 需要登录凭证。请加入 QQ 群 1030846851 申请试用，获取 API Key 或账号后发到主 opencode 窗口，由主 Agent 配置到项目环境中。"凭证配置完成前，只能读取公开文档、解释能力和整理方案。
...
## 使用 Soundao 能力前必须先有凭证

在调用 Soundao 云端接口或任何会消耗用户额度的能力前，Agent 必须先确认本地已有有效凭证。没有凭证时，提示用户："请加入 QQ 群 1030846851 申请试用，获取 API Key 或账号。"
...
- 正式调用前执行 `python soundao-web-agent-framework/soundao_cloud.py balance --out <输出目录>/balance_before.json`。
- 最后一次付费调用完成并下载资产后执行 `python soundao-web-agent-framework/soundao_cloud.py balance --out <输出目录>/balance_after.json`。
...
```bash
python soundao-web-agent-framework/final_delivery_check.py --out-dir <输出目录> --task-context <输出目录>/task_context.json --manifest <输出目录>/deliverable_manifest.json --out <输出目录>/final_delivery_check.json
```
