...
如果本地没有可用用户凭证，Agent 不要尝试调用会消耗额度或需要登录的接口。应先通过页面进度提示告诉用户：请把 Soundao 登录凭证，也就是用户名和密码，发到主 Codex 窗口，由主 Agent 配置到项目环境中；如果还没有凭证，请加入 QQ 群 `1030846851` 申请，申请注明 `Soundao试用`。凭证配置完成前，只能读取公开文档、解释能力和整理方案。
