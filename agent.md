...
Agent 启动后第一件事，必须检查本地是否已有 Soundao 登录凭证；如果没有，先请用户把 Soundao 用户名和密码发到主 Codex 窗口，由主 Agent 配置到项目环境中。用户还没有凭证时，提示用户加入 QQ 群 `1030846851` 申请试用，申请注明 `Soundao试用`。
...
如果本地没有可用用户凭证，Agent 不要尝试调用会消耗额度或需要登录的接口。凭证配置完成前，只能读取公开文档、解释能力和整理方案。
...
- Soundao 登录凭证，即用户名和密码，由 Agent 换取 JWT Token。
- 如果用户还没有凭证，提示用户加入 QQ 群 `1030846851` 申请，申请注明 `Soundao试用`。

凭证处理规则：

- 不要把凭证写进代码、页面、文档、日志或 Git 提交。
- 优先让主 Agent 把凭证配置到本地 `.env`，使用 `SOUNDAO_USER` 和 `SOUNDAO_PASS`。
